...
const ChartCard: React.FC<ChartCardProps> = ({ title, data, total, onTitleClick }) => {
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg">
            {onTitleClick ? (
                <button 
                    ...
                    className="w-full text-left flex justify-between items-center mb-4 group"
                    aria-label={`Lihat detail untuk ${title}`}
                >
                    <h3 className="font-bold text-lg text-gray-800 group-hover:text-indigo-600 transition-colors">{title}</h3>
                    <div className="flex items-center text-sm font-semibold text-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        Lihat Peta
...
const DashboardSummary: React.FC<DashboardSummaryProps> = ({ employees, criticalJobs, onNavigateToTalentPool }) => {
    const summary = useMemo(() => {
        // Dashboard summary should only consider approved employees
        ...
        const boxCounts = approvedEmployees.reduce((acc, emp) => {
            const { boxNumber } = getEmployeeBoxInfo(emp);
            acc[boxNumber] = (acc[boxNumber] || 0) + 1;
            return acc;
        }, {} as { [key: number]: number });
        
        const successionStatusCounts = approvedEmployees.reduce((acc, emp) => {
        ...
        const topTalents = approvedEmployees
        ...
        return { totalEmployees, totalCriticalJobs, totalVacancies, readyNowCount, boxCounts, successionStatusCounts, topTalents, approachingRetirementCount };
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                 <div className="lg:col-span-3 space-y-6">
                    <ChartCard {...boxChartData} onTitleClick={onNavigateToTalentPool} />
