...
import { SuccessionStatus } from '../types';
...
interface HeaderProps {
    ...
    successionStatusFilter: SuccessionStatus | 'all';
    onSuccessionStatusFilterChange: (status: SuccessionStatus | 'all') => void;
    skpdFilter?: string;
    onSkpdFilterChange?: (skpd: string) => void;
    onImportData: () => void;
...
const Header: React.FC<HeaderProps> = ({ 
    ...
    sortKey, 
    onSortKeyChange, 
    successionStatusFilter, 
    onSuccessionStatusFilterChange, 
    skpdFilter = 'all',
    onSkpdFilterChange,
    onImportData, 
    onDownloadTemplate,
                     ...
                     {showControls && (
                        <>
                            ...
                            <div className="relative w-full sm:w-auto">
                                <select
                                    value={successionStatusFilter}
                                    onChange={(e) => onSuccessionStatusFilterChange(e.target.value as SuccessionStatus | 'all')}
                                    ...
                                    aria-label="Filter berdasarkan status suksesi"
                                >
                                    <option value="all">Semua Status</option>
                                    <option value="Siap Sekarang">Siap Sekarang</option>
                                    <option value="1-2 Tahun">1-2 Tahun</option>
                                    <option value="Potensi Masa Depan">Potensi Masa Depan</option>
                                    <option value="Bukan Kandidat">Bukan Kandidat</option>
                                </select>
                            ...
                            {onSkpdFilterChange && (
                                <div className="relative w-full sm:w-auto md:w-48">
                                    <select
                                        value={skpdFilter}
                                        onChange={(e) => onSkpdFilterChange(e.target.value)}
                                        className="appearance-none w-full bg-white text-gray-700 font-semibold px-4 py-2 pr-8 rounded-lg border border-gray-300 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200 shadow-sm whitespace-nowrap overflow-hidden text-ellipsis"
                                        aria-label="Filter berdasarkan SKPD"
                                    >
                                        <option value="all">Semua SKPD</option>
                                        {skpdOptions.map(skpd => (
                                            <option key={skpd} value={skpd}>{skpd}</option>
                                        ))}
                                    </select>
                                    <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                                        <SelectorIcon className="h-4 w-4" />
                            ...
                            {isAdmin && (
                                <>
                                    <button
                                        ...
                                        className="w-full sm:w-auto flex items-center justify-center bg-white border border-gray-300 text-gray-700 font-semibold px-4 py-2 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200 shadow-sm hover:shadow"
