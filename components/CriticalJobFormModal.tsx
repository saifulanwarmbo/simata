...
const CriticalJobFormModal: React.FC<CriticalJobFormModalProps> = ({ isOpen, onClose, onSave, job }) => {
        ...
        if (isOpen) {
            if (job) {
                setFormData({
                    ...
                    requiredGelar: job.requiredGelar || [],
                });
            } else {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        if (name === 'requiredEducation') {
            const educationArray = value.split(',').map(s => s.trim()).filter(Boolean);
            setFormData(prev => ({ ...prev, requiredEducation: educationArray }));
        } else if (name === 'requiredGelar') {
            const gelarArray = value.split(',').map(s => s.trim()).filter(Boolean);
            setFormData(prev => ({ ...prev, requiredGelar: gelarArray }));
        }
        else {
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 max-h-[70vh] overflow-y-auto">
                        ...
                        <div className="md:col-span-2">
                         ...
                         <div className="md:col-span-2">
                            <label htmlFor="requiredEducation" className="block text-sm font-medium text-gray-700">Kualifikasi Jurusan (pisahkan dengan koma)</label>
                            <input
                                type="text"
                                name="requiredEducation"
                                id="requiredEducation"
                                value={formData.requiredEducation.join(', ')}
                                onChange={handleChange}
                                className={inputStyle}
                                placeholder="Contoh: Ilmu Hukum, Manajemen, Akuntansi"
                            />
                        </div>
                         <div className="md:col-span-2">
                            <label htmlFor="requiredGelar" className="block text-sm font-medium text-gray-700">Kualifikasi Gelar Akademik (pisahkan koma)</label>
                            <input
                                type="text"
                                name="requiredGelar"
                                id="requiredGelar"
                                value={formData.requiredGelar?.join(', ') || ''}
                                onChange={handleChange}
                                className={inputStyle}
                                placeholder="Contoh: S.H., S.IP., M.M."
                            />
                        </div>
                         <div className="md:col-span-2">
