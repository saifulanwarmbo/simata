...
const EmployeeFormModal: React.FC<EmployeeFormModalProps> = ({ isOpen, onClose, onSave, onDelete, employee, allEmployees = [], isAdmin = true }) => {
    ...
    }, [formData.performance, formData.potential, formData.nip, formData.eselon, formData.pendidikan, isOpen]);
    ...
    const handleRemoveItem = (field: keyof Omit<Employee, 'skills'>, id: string) => {
        setFormData(prev => ({ ...prev, [field]: (prev[field] as any[]).filter(item => item.id !== id) }));
    };

    const handleArrayItemChange = (field: keyof Omit<Employee, 'skills'>, id: string, e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        const { name, value } = e.target;
        setFormData(prev => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const newEmployeeData: Omit<Employee, 'id' | 'avatar' | 'submissionType' | 'status'> = {
            ...formData,
            unitKerja: formData.unitKerja === 'Lainnya' ? (formData as any).unitKerjaLainnya || '' : (formData as any).unitKerja,
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 max-h-[70vh] overflow-y-auto">
                        ...
                        <div>
                            ...
                            {formData.unitKerja === 'Lainnya' && (
                                <input type="text" name="unitKerjaLainnya" id="unitKerjaLainnya" value={(formData as any).unitKerjaLainnya || ''} onChange={handleChange} placeholder="Sebutkan instansi lainnya" required className={`${inputStyle} mt-2`} />
                            )}
                        </div>
