            ...
            {boxNumber === null ? (
                ...
                >
                    <div className="border-b pb-4 mb-4">
                        <h2 className="text-xl font-bold text-gray-800">{`Kotak ${boxNumber}: ${categoryMap[boxNumber] || 'Kategori Tidak Dikenal'}`}</h2>
                        <p className="text-sm text-gray-600 mt-1">{`(${employees.length} Pegawai)`}</p>
                        <p className="text-xs text-indigo-700 font-semibold bg-indigo-50 p-2 rounded-md mt-3">{`Rekomendasi: ${recommendationMap[boxNumber] || 'Tidak ada rekomendasi.'}`}</p>
                    </div>
                        ...
                        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                            {employees.map((employee, idx) => (
                                <motion.div 
                                    ...
                                    className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 hover:shadow-sm hover:border-indigo-200 transition-all duration-200"
                                >
