...
const DevelopmentPlanModal: React.FC<DevelopmentPlanModalProps> = ({ isOpen, onClose, title, content, isLoading, error, employeeName }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm z-40 flex justify-center items-center" onClick={onClose}>
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="bg-white rounded-xl shadow-2xl w-full max-w-3xl m-4 flex flex-col" onClick={e => e.stopPropagation()}>
